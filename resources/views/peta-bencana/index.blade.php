...
<div class="max-w-7xl mx-auto px-6 py-8">
    ...
    <div class="flex justify-between items-start mb-6">
        ...
        <a href="{{ route('kejadian-bencana.create') }}"
           class="bg-green-700 hover:bg-green-800 text-white px-5 py-2.5 rounded-full font-medium">
            + Lapor Kejadian
        </a>
    </div>
    ...
    <div class="flex gap-6">
        ...
        <div class="w-72 shrink-0 space-y-4">
            ...
            <div class="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
                @php
                    $jenis = [
                        ['label' => 'Banjir', 'icon' => 'icon-banjir.svg', 'checked' => true],
                        ['label' => 'Kebakaran', 'icon' => 'icon-kebakaran.svg', 'checked' => false],
                        ['label' => 'Badai Angin', 'icon' => 'icon-putingbeliung.svg', 'checked' => true],
                        ['label' => 'Longsor', 'icon' => 'icon-longsor.svg', 'checked' => false],
                    ];
                @endphp
                @foreach($jenis as $j)
                    <div class="flex items-center justify-between" x-data="{ on: {{ $j['checked'] ? 'true' : 'false' }} }">
            ...
            <div class="bg-green-800 text-white rounded-xl p-5">
                ...
                <p class="text-3xl font-bold mb-3">20</p>
                ...
                <div class="space-y-3">
                    @forelse($kejadianTerbaru as $kejadian)
                        <div class="flex items-start gap-2">
                            ...
                            <div>
                                <p class="text-xs text-gray-400">{{ $kejadian->tanggal_kejadian->diffForHumans() }}</p>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const map = L.map('peta-bencana', { zoomControl: false }).setView([-3.3194, 114.5908], 8);
