...
<div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
    ...
    <div class="flex justify-between items-center px-6 py-5">
        ...
        <div class="flex items-center gap-3">
            <input type="text" placeholder="Cari lokasi/jenis..." class="border border-gray-300 rounded-lg px-4 py-2 text-sm w-64">
            <a href="{{ route('admin.input-kejadian') }}" class="bg-green-700 hover:bg-green-800 text-white px-4 py-2 rounded-lg text-sm font-medium">
                Tambah Data
            </a>
        </div>
    </div>
    ...
    <table class="w-full text-left text-sm">
        <thead>
            <tr class="text-gray-500 border-y border-gray-100 bg-gray-50 uppercase text-xs">
        ...
        <tbody>
            ...
            <tr class="border-b border-gray-50">
                ...
                <td class="px-6 py-3.5">
                    ...
                    @else
                    <span class="bg-yellow-50 text-yellow-700 text-xs font-medium px-2.5 py-1 rounded-full">Pending</span>
                    @endif
                </td>
                <td class="px-6 py-3.5">
                    <div class="flex items-center gap-2">
                        ...
                        <a href="{{ route('kejadian-bencana.edit', $item->id) }}" class="text-blue-600 hover:text-blue-800" title="Edit">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <form action="{{ route('kejadian-bencana.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                            @csrf
                            @method('DELETE')
