...
@section('page-title', 'Form Input Berita & Artikel Edukasi')
...
<div class="w-full">
    ...
    <div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
        ...
        <div class="px-6 py-5 border-b border-gray-100">
            <h2 class="text-lg font-semibold text-gray-900">Form Publikasi Informasi & Artikel Bencana</h2>
            <p class="text-sm text-gray-500 mt-1">
                Lengkapi informasi berita berikut untuk dipublikasikan pada portal informasi masyarakat SIPANTAU.
            </p>
        </div>

        <form action="{{ route('berita.store') }}" method="POST" enctype="multipart/form-data" class="p-6" id="formBerita">
            @csrf
            ...
            <div class="mb-5">
                ...
                <input type="text" id="judul" name="judul" value="{{ old('judul') }}" placeholder="Masukkan judul berita/artikel" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500" required>
                @error('judul')
            ...
            <div class="grid grid-cols-2 gap-5 mb-5">
                <div>
                    ...
                    <input type="datetime-local" id="tanggal" name="tanggal" value="{{ old('tanggal', now()->format('Y-m-d\TH:i')) }}" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500" required>
                    @error('tanggal')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select id="kategori" name="kategori" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500">
                        ...
                        <option value="Mitigasi" {{ old('kategori') == 'Mitigasi' ? 'selected' : '' }}>Mitigasi</option>
            ...
            {{-- Gambar Utama / Thumbnail (drag & drop, otomatis jadi 2 kolom saat ada gambar) --}}
            <div class="mb-5">
                <label class="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-2">
                    Gambar Utama Berita / Thumbnail <span class="text-red-500">*</span>
                </label>

                <div class="grid grid-cols-2 gap-4" id="uploadGrid">
                    <div id="dropZone" class="col-span-2 h-36 border-2 border-dashed border-green-300 bg-green-50/30 rounded-xl flex flex-col items-center justify-center text-center px-4 cursor-pointer hover:bg-green-50 transition">
                    ...
                    <div id="previewBox" class="hidden h-36 relative rounded-xl overflow-hidden border border-gray-200">
            ...
            <div class="mb-2">
                ...
                <textarea name="isi" id="isiHidden" class="hidden">{{ old('isi') }}</textarea>
                @error('isi')
                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-between items-center mt-6">
                <button type="button" id="resetForm" class="px-4 py-2.5 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
